...
func SetupTestScoreHandler(t *testing.T) ScoreHandlerTestDeps {
	...
	// Clean score-specific streams
	scoreStreams := []string{"user", "discord", "leaderboard", "round", "score", "delayed"}
	if err := env.ResetJetStreamState(env.Ctx, scoreStreams...); err != nil {
	...
	for _, streamName := range scoreStreams {
	...
	cleanup := func() {
		...
		if scoreModule != nil {
			if err := scoreModule.Close(); err != nil {
				log.Printf("Error closing Score module: %v", err)
			}
		}
