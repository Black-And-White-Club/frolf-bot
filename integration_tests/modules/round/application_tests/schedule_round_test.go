...
func TestScheduleRoundEvents_Behavioral(t *testing.T) {
	tests := []struct {
		name          string
		setupRound    func() (roundevents.RoundScheduledPayloadV1, string)
		expectedError bool
		waitAndVerify func(t *testing.T, ctx context.Context, deps RoundTestDeps, payload roundevents.RoundScheduledPayloadV1)
	}{
		{
			...
			setupRound: func() (roundevents.RoundScheduledPayloadV1, string) {
				...
				// But the start time needs to be at least 5 seconds in the future to be scheduled
				futureTime := sharedtypes.StartTime(time.Now().UTC().Add(8 * time.Second))
			...
			waitAndVerify: func(t *testing.T, ctx context.Context, deps RoundTestDeps, payload roundevents.RoundScheduledPayloadV1) {
				...
				go func() {
					...
					subReminder, err := deps.EventBus.Subscribe(ctx, string(roundevents.RoundReminderScheduledV1))
					if err != nil {
					...
					case msg := <-subReminder:
						var reminderPayload roundevents.DiscordReminderPayloadV1
					...
					case <-time.After(12 * time.Second): // Wait a bit longer than expected start time
						reminderNotReceived <- true // Confirmed no reminder received
					}
				}()

				timeout := 12 * time.Second // A bit longer than the scheduled start time
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result, err := deps.Service.ScheduleRoundEvents(deps.Ctx, sharedtypes.GuildID("test-guild"), payload, discordMessageID)
			...
			scheduledPayload, ok := result.Success.(*roundevents.RoundScheduledPayloadV1)
			if !ok {
				t.Errorf("Expected result to be *RoundScheduledPayloadV1, got %T", result.Success)
				return
			}
