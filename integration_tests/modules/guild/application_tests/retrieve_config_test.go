	...
	tests := []struct {
	...
	}{
		{
			...
			validateFn: func(t *testing.T, deps TestDeps, guildID sharedtypes.GuildID, result results.OperationResult[*guildtypes.GuildConfig, error], err error) {
				...
				if successPayload.SignupChannelID != "834567890123456789" {
					t.Errorf("Config SignupChannelID mismatch: expected %q, got %q", "834567890123456789", successPayload.SignupChannelID)
				}
				if successPayload.EventChannelID != "845678901234567890" {
					t.Errorf("Config EventChannelID mismatch: expected %q, got %q", "845678901234567890", successPayload.EventChannelID)
				}
			},
		},
		{
			...
			validateFn: func(t *testing.T, deps TestDeps, guildID sharedtypes.GuildID, result results.OperationResult[*guildtypes.GuildConfig, error], err error) {
				...
				// Verify specific error if possible, or just that a failure occurred
				// In the implementation, it returns ErrGuildConfigNotFound which is a domain error
			},
		},
		{
