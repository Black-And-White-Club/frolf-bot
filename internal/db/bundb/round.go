...
// CreateRoundScore creates a new round score in the database.
func (r *roundDB) CreateRoundScore(ctx context.Context, roundScore *models.RoundScore) error {
	_, err := r.db.NewInsert().
		Model(roundScore).
		Exec(ctx)
	if err != nil {
		return fmt.Errorf("failed to create round score: %w", err)
	}
	return nil
}
