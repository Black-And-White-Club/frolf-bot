...
func (h *LeaderboardHandlers) HandleCheckTagAvailabilityRequest(ctx context.Context, msg *message.Message) error {
	...
	// Get the reply subject from the request message metadata
	replySubject := msg.Metadata.Get("Reply-To")
	if replySubject == "" {
		h.logger.Error("Missing Reply-To metadata in request", nil, watermill.LogFields{
			"message_id": msg.UUID,
		})
		return fmt.Errorf("missing Reply-To metadata in request")
	}

	h.logger.Info("Publishing CheckTagAvailabilityResponse", watermill.LogFields{
		"message_id":   respMsg.UUID,
		"replySubject": replySubject, // Log the reply subject
		"is_available": resp.IsAvailable,
	})

	// Publish the response on the replySubject
	if err := h.Publisher.Publish(replySubject, respMsg); err != nil {
