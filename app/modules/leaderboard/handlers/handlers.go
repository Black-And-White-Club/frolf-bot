...
func (h *LeaderboardHandlers) HandleLeaderboardUpdate(ctx context.Context, msg *message.Message) error {
	h.logger.Info("HandleLeaderboardUpdate called", watermill.LogFields{"message_id": msg.UUID})

	var event leaderboardevents.LeaderboardUpdateEvent
	if err := json.Unmarshal(msg.Payload, &event); err != nil {
		h.logger.Error("Failed to unmarshal LeaderboardUpdateEvent", err, watermill.LogFields{"message_id": msg.UUID})
	...
	if err != nil {
		h.logger.Error("Failed to process leaderboard update", err, watermill.LogFields{"message_id": msg.UUID})
		return fmt.Errorf("failed to process leaderboard update: %w", err)
	}

	h.logger.Info("HandleLeaderboardUpdate completed", watermill.LogFields{"message_id": msg.UUID})
	return nil
}
...
func (h *LeaderboardHandlers) HandleTagAssigned(ctx context.Context, msg *message.Message) error {
	h.logger.Info("HandleTagAssigned called", watermill.LogFields{"message_id": msg.UUID})

	var event leaderboardevents.TagAssigned
	if err := json.Unmarshal(msg.Payload, &event); err != nil {
		h.logger.Error("Failed to unmarshal TagAssigned", err, watermill.LogFields{"message_id": msg.UUID})
	...
	if err != nil {
		h.logger.Error("Failed to process tag assignment", err, watermill.LogFields{"message_id": msg.UUID})
		return fmt.Errorf("failed to process tag assignment: %w", err)
	}

	h.logger.Info("HandleTagAssigned completed", watermill.LogFields{"message_id": msg.UUID})
	return nil
}
...
func (h *LeaderboardHandlers) HandleTagSwapRequest(ctx context.Context, msg *message.Message) error {
	h.logger.Info("HandleTagSwapRequest called", watermill.LogFields{"message_id": msg.UUID})

	var event leaderboardevents.TagSwapRequest
	if err := json.Unmarshal(msg.Payload, &event); err != nil {
		h.logger.Error("Failed to unmarshal TagSwapRequest", err, watermill.LogFields{"message_id": msg.UUID})
	...
	if err != nil {
		h.logger.Error("Failed to process tag swap request", err, watermill.LogFields{"message_id": msg.UUID})
		return fmt.Errorf("failed to process tag swap request: %w", err)
	}

	h.logger.Info("HandleTagSwapRequest completed", watermill.LogFields{"message_id": msg.UUID})
	return nil
}
...
func (h *LeaderboardHandlers) HandleGetLeaderboardRequest(ctx context.Context, msg *message.Message) error {
	h.logger.Info("HandleGetLeaderboardRequest called", watermill.LogFields{"message_id": msg.UUID})
	...
	if err != nil {
		h.logger.Error("Failed to get leaderboard", err, watermill.LogFields{"message_id": msg.UUID})
	...
	if err != nil {
		h.logger.Error("Failed to marshal GetLeaderboardResponse", err, watermill.LogFields{"message_id": msg.UUID})
	...
	if err := h.Publisher.Publish(leaderboardevents.GetLeaderboardResponseSubject, message.NewMessage(watermill.NewUUID(), respData)); err != nil {
		h.logger.Error("Failed to publish GetLeaderboardResponse", err, watermill.LogFields{"message_id": msg.UUID})
		return fmt.Errorf("failed to publish GetLeaderboardResponse: %w", err)
	}

	h.logger.Info("HandleGetLeaderboardRequest completed", watermill.LogFields{"message_id": msg.UUID})
	return nil
}
...
func (h *LeaderboardHandlers) HandleGetTagByDiscordIDRequest(ctx context.Context, msg *message.Message) error {
	h.logger.Info("HandleGetTagByDiscordIDRequest called", watermill.LogFields{"message_id": msg.UUID})

	var req leaderboardevents.GetTagByDiscordIDRequest
	if err := json.Unmarshal(msg.Payload, &req); err != nil {
		h.logger.Error("Failed to unmarshal GetTagByDiscordIDRequest", err, watermill.LogFields{"message_id": msg.UUID})
	...
	if err != nil {
		h.logger.Error("Failed to get tag by Discord ID", err, watermill.LogFields{"message_id": msg.UUID})
	...
	respData, err := json.Marshal(resp)
	if err != nil {
		h.logger.Error("Failed to marshal GetTagByDiscordIDResponse", err, watermill.LogFields{"message_id": msg.UUID})
	...
	if err := h.Publisher.Publish(leaderboardevents.GetTagByDiscordIDResponseSubject, message.NewMessage(watermill.NewUUID(), respData)); err != nil {
		h.logger.Error("Failed to publish GetTagByDiscordIDResponse", err, watermill.LogFields{"message_id": msg.UUID})
		return fmt.Errorf("failed to publish GetTagByDiscordIDResponse: %w", err)
	}

	h.logger.Info("HandleGetTagByDiscordIDRequest completed", watermill.LogFields{"message_id": msg.UUID})
	return nil
}
...
func (h *LeaderboardHandlers) HandleCheckTagAvailabilityRequest(ctx context.Context, msg *message.Message) error {
	h.logger.Info("HandleCheckTagAvailabilityRequest called", watermill.LogFields{"message_id": msg.UUID})

	var req leaderboardevents.CheckTagAvailabilityRequest
	if err := json.Unmarshal(msg.Payload, &req); err != nil {
		h.logger.Error("Failed to unmarshal CheckTagAvailabilityRequest", err, watermill.LogFields{"message_id": msg.UUID})
	...
	if err != nil {
		h.logger.Error("Failed to check tag availability", err, watermill.LogFields{"message_id": msg.UUID})
		return fmt.Errorf("failed to check tag availability: %w", err)
	}

	// Publish the response event
	resp := leaderboardevents.CheckTagAvailabilityResponse{IsAvailable: isAvailable}
	respData, err := json.Marshal(resp)
	if err != nil {
		h.logger.Error("Failed to marshal CheckTagAvailabilityResponse", err, watermill.LogFields{"message_id": msg.UUID})
		return fmt.Errorf("failed to marshal CheckTagAvailabilityResponse: %w", err)
	}

	if err := h.Publisher.Publish(leaderboardevents.CheckTagAvailabilityResponseSubject, message.NewMessage(watermill.NewUUID(), respData)); err != nil {
		h.logger.Error("Failed to publish CheckTagAvailabilityResponse", err, watermill.LogFields{"message_id": msg.UUID})
		return fmt.Errorf("failed to publish CheckTagAvailabilityResponse: %w", err)
	}

	h.logger.Info("HandleCheckTagAvailabilityRequest completed", watermill.LogFields{"message_id": msg.UUID})
	return nil
}
