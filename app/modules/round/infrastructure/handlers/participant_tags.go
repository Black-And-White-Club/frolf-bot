...
func (h *RoundHandlers) HandleScheduledRoundTagUpdate(
	...
	result, err := h.roundService.UpdateScheduledRoundsWithNewTags(
		ctx,
		payload.GuildID,
		payload.ChangedTags,
	)
	...
	// 2. Handle Service-Level Failures
	// Following your 'proper structure' example, we check the Failure payload.
	if result.Failure != nil {
		h.logger.WarnContext(ctx, "Round tag update service failure",
			attr.String("guild_id", string(payload.GuildID)),
			attr.Any("failure", result.Failure),
		)
		// We return the failure result so the system can emit a failure event if needed
	...
	// 3. Success Path: Return the payload generated by the service
	// The service already builds the TagsUpdatedForScheduledRoundsPayloadV1,
	// so we can pass it straight to the event bus.
	if result.Success != nil {
		return []handlerwrapper.Result{
			{
				Topic:   roundevents.TagsUpdatedForScheduledRoundsV1,
