...
func TestGuildHandlers_HandleGuildSetup(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			setupFake: func(f *FakeGuildService) {
				f.CreateGuildConfigFunc = func(ctx context.Context, config *guildtypes.GuildConfig) (guildservice.GuildConfigResult, error) {
					// Return success with the provided config
		...
		{
			name: "failure - guild config already exists",
			payload: &guildtypes.GuildConfig{
				GuildID: sharedtypes.GuildID("guild-1"),
			},
			setupFake: func(f *FakeGuildService) {
				f.CreateGuildConfigFunc = func(ctx context.Context, config *guildtypes.GuildConfig) (guildservice.GuildConfigResult, error) {
					// Simulate domain failure (conflict)
					return results.FailureResult[*guildtypes.GuildConfig, error](guildservice.ErrGuildConfigConflict), nil
				}
			},
			wantErr:   false,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			res, err := h.HandleGuildSetup(context.Background(), tt.payload)
